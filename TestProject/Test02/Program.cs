...
namespace Test02 {
    ...
    class Program {
        ...
        private static void Exercise03(List<int> numbers) {
            var num2 = numbers.Where(n=> n % 2 == 1).OrderByDescending(n=> n);
        ...
        private static void Exercise04(List<int> numbers) {
            var selnum = numbers.Where(n => n >= 10).Where(n => n <= 50);
            foreach(var num in selnum) {
                Console.Write(num + " ");
            }
        ...
        private static void Exercise06(List<Person> persons) {
            var weight = persons.Where(n => n.Height >= 170).Average(n=> n.Weight);
        ...
        private static void Exercise07(List<Person> persons) {
            var serch = persons.Where(s => s.Weight <= 70);
            foreach(var serch2 in serch) {
                Console.WriteLine(serch2.Name + " " + serch2.Weight + "kg");
            }
        ...
        private static void Exercise08(List<Person> persons) {
            var sernum = persons.Where(s => s.Name.Contains("山"));
            foreach(var sernum2 in sernum) {
                Console.WriteLine(sernum2.Name);
            }
